...
public:
    ...
    Token  next();
    Token  peek();
    bool   at_eof() const { return pos_ >= size_; }
    size_t pos()    const { return pos_; }
    void   seek(size_t p) { pos_ = p; has_peek_ = false; }

    // raw bytes 접근 (stream 데이터 추출용)
    const uint8_t* buf()  const { return buf_; }
    size_t         bsize() const { return size_; }
...
private:
    ...
    uint8_t cur()      const { return pos_ < size_ ? buf_[pos_] : 0; }
    bool    is_ws(uint8_t c)    const;
    bool    is_delim(uint8_t c) const;
    bool    is_regular(uint8_t c) const { return !is_ws(c) && !is_delim(c); }
};
