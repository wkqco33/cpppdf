...
static void parse_xref_table(Lexer& lex, XRefTable& table) {
    while (true) {
        ...
        for (int32_t k = 0; k < count; k++) {
            ...
            if (offset_tok.type != TokType::Int ||
                gen_tok.type    != TokType::Int ||
                type_tok.type   != TokType::Keyword) break;
...
static bool parse_xref_stream(const uint8_t* data, size_t size,
                               size_t offset, XRefTable& table, PdfDict& trailer) {
    ...
    if (t3.type != TokType::Keyword || t3.sv != "obj") return false;

    Token dictBegin = lex.next();
    if (dictBegin.type != TokType::DictBegin) return false;

    PdfObject obj = parse_dict_or_stream(lex);
    if (!obj.is_stream() || !obj.stream) return false;

    // stream raw 데이터 읽기
    size_t stream_pos = lex.pos();
    // stream 키워드 뒤 줄바꿈 건너뜀
    while (stream_pos < size &&
           (data[stream_pos] == '\r' || data[stream_pos] == '\n'))
        ++stream_pos;
    ...
    if (!decode_stream(*obj.stream)) return false;
    const auto& raw = obj.stream->decoded;

    // /W: 각 필드 너비 [type, offset, gen]
    auto wit = sd.find("W");
    if (wit == sd.end() || !wit->second.is_array()) return false;
    const auto& warr = wit->second.arr;
    if (warr.size() < 3) return false;
    ...
    for (auto [start, count] : sections) {
        for (int32_t k = 0; k < count; k++) {
            if (byte_pos + static_cast<size_t>(entry_size) > raw.size()) break;
...
XRefResult parse_xref(const uint8_t* data, size_t size, size_t xref_offset) {
    XRefResult result;
    if (xref_offset == 0 || xref_offset >= size) return result;
    ...
    if (t.type == TokType::Keyword && t.sv == "xref") {
        ...
        // xref stream
        result.ok = parse_xref_stream(data, size, xref_offset,
                                       result.table, result.trailer);
    }
