...
PdfObject parse_value(Lexer &lex) {
    ...
    case TokType::Bool:
        return PdfObject::from_bool(t.bv);
    case TokType::Real:
        return PdfObject::from_real(t.rv);
    case TokType::StrLit:
    case TokType::StrHex:
        return PdfObject::from_string(t.sv);
    case TokType::Name:
        return PdfObject::from_name(t.sv);
    case TokType::ArrBegin:
        return PdfObject::from_array(parse_array(lex));
    case TokType::DictBegin:
        return parse_dict_or_stream(lex);
    ...
    case TokType::Int: {
        ...
        if (t2.type == TokType::Int) {
            ...
            if (t3.type == TokType::Keyword && t3.sv == "R") {
                lex.next(); // R 소비
                return PdfObject::from_ref(static_cast<int32_t>(t.iv), static_cast<int32_t>(t2.iv));
            }
            // R이 아님: t2 위치로 되돌림
            lex.seek(saved);
        }
...
bool apply_png_predictor(std::vector<uint8_t> &data, int columns, int colors, int bpc) {
    int bytes_per_pixel = (colors * bpc + 7) / 8;
    int row_bytes = (columns * colors * bpc + 7) / 8;
    int stride = row_bytes + 1; // filter byte + row data

    if (stride <= 0 || data.size() < static_cast<size_t>(stride))
        return false;
    ...
    for (size_t row = 0; row < num_rows; row++) {
        ...
        for (int col = 0; col < row_bytes; col++) {
            ...
            uint8_t c =
                (col >= bytes_per_pixel) ? prev[static_cast<size_t>(col - bytes_per_pixel)] : 0;
            uint8_t x = in_r[col];
            ...
            case 0:
                ...
                break; // None
            case 1:
                out_r[col] = static_cast<uint8_t>(x + a);
                break; // Sub
            case 2:
                out_r[col] = static_cast<uint8_t>(x + b);
                break; // Up
            case 3:
                out_r[col] = static_cast<uint8_t>(x + (a + b) / 2);
                break; // Average
            case 4: {  // Paeth
