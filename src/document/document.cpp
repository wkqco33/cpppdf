...
class PdfDocumentImpl {
  ...
  private:
    ...
    PdfDict trailer_;
    mutable std::map<std::pair<int32_t, int32_t>, PdfObject> obj_cache_;
    ...
    PdfObject parse_object_from_stream(int32_t object_stream_num, int32_t target_num,
...
bool PdfDocumentImpl::load(const std::string &path) {
    ...
    // 페이지 트리 구축
    PdfObject catalog =
        resolve(trailer_.count("Root") ? trailer_.at("Root") : PdfObject::null_obj());
    if (!catalog.is_dict())
        return fail();
...
void PdfDocumentImpl::fill_stream_raw(PdfStream &stream, size_t stream_kw_pos) const {
    ...
    if (length <= 0 || static_cast<size_t>(length) > size - pos)
        return;
...
PdfObject PdfDocumentImpl::parse_object(int32_t num) const {
    ...
    if (it == xref_.end() || !it->second.in_use)
        return PdfObject::null_obj();

    if (it->second.type == parser::XRefEntryType::Compressed) {
        return parse_object_from_stream(it->second.obj_stream_num, num,
...
PdfObject PdfDocumentImpl::parse_object_from_stream(int32_t object_stream_num, int32_t target_num,
                                                    int32_t target_index) const {
    ...
    for (int i = 0; i < object_count; ++i) {
        ...
        if (num_tok.type != parser::TokType::Int || off_tok.type != parser::TokType::Int) {
            return PdfObject::null_obj();
        }
        index.emplace_back(static_cast<int32_t>(num_tok.iv), static_cast<int32_t>(off_tok.iv));
    }
    ...
    if (index[static_cast<size_t>(resolved_index)].first != target_num) {
        ...
        if (resolved_index < 0 || index[static_cast<size_t>(resolved_index)].first != target_num) {
            return PdfObject::null_obj();
        }
    }
...
PdfObject PdfDocumentImpl::get_object(int32_t num, int32_t gen) const {
    const auto key = std::make_pair(num, gen);
    auto it = obj_cache_.find(key);
    if (it != obj_cache_.end())
        return it->second;

    auto xref_it = xref_.find(num);
    if (xref_it == xref_.end() || xref_it->second.gen != gen) {
        return PdfObject::null_obj();
    }
...
void PdfDocumentImpl::build_page_list(const PdfObject &node, PdfDict inherited_resources,
                                      std::array<float, 4> inherited_media_box) {
    if (!node.is_dict())
        return;
    ...
    if (res_it != d.end()) {
        ...
        if (res.is_dict())
            inherited_resources = res.as_dict();
    }
    ...
    if (mb_it != d.end()) {
        ...
        if (mb.is_array() && mb.arr.size() >= 4) {
            for (int k = 0; k < 4; k++) {
                ...
                inherited_media_box[static_cast<size_t>(k)] = static_cast<float>(v.as_number());
            }
        }
    }
    ...
    if (type == "Pages") {
        ...
        PdfObject page = node;
        PdfDict &page_dict = page.as_dict();
        if (page_dict.count("Resources") == 0 && !inherited_resources.empty()) {
            page_dict["Resources"] = PdfObject::from_dict(inherited_resources);
        }
        if (page_dict.count("MediaBox") == 0) {
            PdfArray media_box;
            for (float value : inherited_media_box)
                media_box.push_back(PdfObject::from_real(value));
            page_dict["MediaBox"] = PdfObject::from_array(std::move(media_box));
        }
...
std::vector<uint8_t> PdfDocumentImpl::get_content(int index) const {
    ...
    auto append_stream = [&](const PdfObject &obj) {
        ...
        if (obj.stream->decoded_ok) {
            result.insert(result.end(), obj.stream->decoded.begin(), obj.stream->decoded.end());
            result.push_back(' ');
        }
    };
