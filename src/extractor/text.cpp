...
struct FontEncoding {
    ...
    FontEncoding() {
        for (int i = 0; i < 256; i++)
            table[static_cast<size_t>(i)] = static_cast<uint32_t>(i);
    }
};
...
static std::string utf16be_to_utf8(const std::string &bytes) {
    ...
    while (pos + 1 < bytes.size()) {
        uint16_t hi =
            (static_cast<uint8_t>(bytes[pos]) << 8) | static_cast<uint8_t>(bytes[pos + 1]);
        pos += 2;
...
static void update_code_width(FontEncoding &enc, size_t width) {
    if (width == 0)
        return;
    enc.min_code_bytes = (enc.min_code_bytes == 0) ? width : std::min(enc.min_code_bytes, width);
...
static FontEncoding parse_to_unicode(const std::vector<uint8_t> &data) {
    ...
    while (!lex.at_eof()) {
        ...
        if (t.type != parser::TokType::Keyword)
            continue;

        if (t.sv == "beginbfchar") {
            while (!lex.at_eof()) {
                parser::Token src = lex.next();
                if (src.type == parser::TokType::Keyword)
                    break; // endbfchar
                if (src.type != parser::TokType::StrHex)
                    continue;
                parser::Token dst = lex.next();
                if (dst.type != parser::TokType::StrHex)
                    continue;
            ...
            while (!lex.at_eof()) {
                ...
                if (lo.type == parser::TokType::Keyword)
                    break; // endbfrange
                parser::Token hi = lex.next();
                parser::Token start = lex.next();
                if (lo.type != parser::TokType::StrHex || hi.type != parser::TokType::StrHex)
                    continue;
                ...
                if (start.type == parser::TokType::StrHex) {
                    ...
                    for (uint32_t c = lo_c; c <= hi_c; c++) {
                        ...
                        if (key.size() == 1)
                            enc.table[static_cast<uint8_t>(key[0])] = cp;
                    }
                } else if (start.type == parser::TokType::ArrBegin) {
                    uint32_t c = lo_c;
                    while (!lex.at_eof()) {
                        parser::Token at = lex.peek();
                        if (at.type == parser::TokType::ArrEnd) {
                            lex.next();
                            break;
                        }
...
static FontEncoding build_font_encoding(const PdfObject &font_obj, const PdfDocument &doc) {
    FontEncoding enc;
    if (!font_obj.is_dict())
        return enc;
    ...
    if (tu_it != fd.end()) {
        ...
        if (tu.is_stream() && tu.stream) {
            if (!tu.stream->decoded_ok)
                parser::decode_stream(*tu.stream);
            if (tu.stream->decoded_ok)
                return parse_to_unicode(tu.stream->decoded);
        }
    }
...
static std::vector<TextBlock> parse_content(const std::vector<uint8_t> &stream,
    ...
    while (!lex.at_eof()) {
        ...
        if (t.type == parser::TokType::ArrBegin || t.type == parser::TokType::DictBegin) {
        ...
        case parser::TokType::Int:
            stack.push_back(PdfObject::from_int(t.iv));
            break;
        case parser::TokType::Real:
            stack.push_back(PdfObject::from_real(t.rv));
            break;
        case parser::TokType::Bool:
            stack.push_back(PdfObject::from_bool(t.bv));
            break;
        case parser::TokType::Null:
            stack.push_back(PdfObject::null_obj());
            break;
        case parser::TokType::StrLit:
        case parser::TokType::StrHex:
            stack.push_back(PdfObject::from_string(t.sv));
            break;
        case parser::TokType::Name:
            stack.push_back(PdfObject::from_name(t.sv));
            break;
        ...
        case parser::TokType::Keyword: {
            ...
            // 텍스트 파라미터
                ...
                state.char_space = pop_real(1);
            } else if (op == "Tw") {
                state.word_space = pop_real(1);
            } else if (op == "Ts") {
                state.rise = pop_real(1);
            }
            ...
            // 텍스트 위치
                ...
                for (int k = 0; k < 6; k++)
                    state.tm[k] =
                        static_cast<float>(stack[sz - 6 + static_cast<size_t>(k)].as_number());
            ...
            // 텍스트 출력
            else if (op == "Tj" && in_text && !stack.empty() && stack.back().is_string()) {
                emit(stack.back().s);
            } else if (op == "'" && in_text && !stack.empty() && stack.back().is_string()) {
                state.next_line();
                emit(stack.back().s);
                ...
                if (stack.back().is_string())
                    emit(stack.back().s);
            } else if (op == "TJ" && in_text && !stack.empty() && stack.back().is_array()) {
                std::string text;
                float x0 = state.x();
                for (const auto &item : stack.back().arr) {
                    if (item.is_string()) {
                        text += cur_enc ? decode_bytes(item.s, *cur_enc) : item.s;
                        state.tm[4] += static_cast<float>(item.s.size()) * state.font_size * 0.5f;
