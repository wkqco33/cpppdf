...
static bool dct_decode(const uint8_t *src, size_t src_size, std::vector<uint8_t> &dst, int &width,
    ...
    while (cinfo.output_scanline < cinfo.output_height) {
        uint8_t *row = dst.data() +
                       static_cast<size_t>(cinfo.output_scanline) * static_cast<size_t>(row_stride);
...
static std::vector<uint8_t> expand_indexed(const PdfObject &cs_obj, const std::vector<uint8_t> &src,
                                           const PdfDocument &doc) {
    // /Indexed /base hival lookup
    if (!cs_obj.is_array() || cs_obj.arr.size() < 4)
        return src;
    ...
    if (lookup_obj.is_string()) {
        ...
        palette.assign(lookup_obj.stream->decoded.begin(), lookup_obj.stream->decoded.end());
    }
...
static ImageData to_rgba(const std::vector<uint8_t> &pixels, int width, int height,
...
static ImageData decode_image_xobject(PdfObject xobj, const PdfDocument &doc) {
    ...
    if (cs_it != dict.end()) {
        ...
        if (colorspace != "DeviceRGB" && colorspace != "DeviceGray" && colorspace != "DeviceCMYK") {
    ...
    if (first_filter == "DCTDecode" || first_filter == "DCT") {
        ...
        if (!dct_decode(stream.raw.data(), stream.raw.size(), decoded, jw, jh, jc))
            return {};
        std::string jcs = (jc == 1) ? "DeviceGray" : (jc == 4) ? "DeviceCMYK" : "DeviceRGB";
...
std::vector<ImageData> extract_images(const PdfDocument &doc, int page_index) {
    ...
    for (const auto &[name, ref] : xobj_map.as_dict()) {
        ...
        if (!subtype.is_name() || subtype.s != "Image")
            continue;
