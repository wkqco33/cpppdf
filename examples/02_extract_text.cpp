...
int main(int argc, char** argv) {
    ...
    int end_page = (target_page >= 0) ? std::min(target_page + 1, doc.page_count()) : doc.page_count();
    ...
    for (int p = start_page; p < end_page; ++p) {
        ...
        std::sort(blocks.begin(), blocks.end(), [](const cpppdf::TextBlock& a, const cpppdf::TextBlock& b) {
